...
public class AtoB : MonoBehaviour
{
    ...
    public float diatanceTotarget;//距離
    ...
    void Update()
    {
        diatanceTotarget=Vector3.Distance(start.position, target.position);
        //if (diatanceTotarget > 5)
        Lerp();
    }
    void Lerp()//以比例算 直線
    {
        transform.position = Vector3.Lerp(start.position, target.position, Time.time * distance);//
    }
    void MoveTowards()////以速度、距離算 直線
    {
        transform.position = Vector3.MoveTowards(start.position, target.position, speed * Time.deltaTime);
    }
    void parabola()//拋物線
    {
        Vector3 center = start.position + target.position / 100;
